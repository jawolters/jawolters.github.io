...
const gradientCorrectionProgram = createProgram(stencilShader, gradientCorrShader);
...
function createFramebufferPair(init) {
    ...
    return {
        get old () { return fb_old; },
        get new () { return fb_new; },
        swapMem () {
            let temp = fb_old;
            fb_old = fb_new;
            fb_new = temp;
...
function addForce(){
    if(pointer.moved && !pointer.down) {
        ...
        gl.uniform2fv(forceParams.force, [pointer.dx*scale, pointer.dy*scale]);
